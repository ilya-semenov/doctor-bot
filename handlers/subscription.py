...
async def support_command(message: Message):
    ...
    try:
        ...
        logging.error(f"Ошибка в support_command: {e}")
...
async def subscription_info(callback: CallbackQuery):
    ...
    try:
        ...
        logging.error(f"Ошибка в subscription_info: {e}")
...
async def handle_stars_payment(callback: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        await bot.send_invoice(
            ...
            description=f"Сумма поддержки: {amount} Stars. Ваша поддержка помогает развивать проект!",
        ...
        logging.error(f"Ошибка в handle_stars_payment: {e}")
...
async def handle_card_payment(callback: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        logging.error(f"Ошибка в handle_card_payment: {e}")
        await callback.message.edit_text(
            f"❌ *Ошибка:* {str(e)}\n\nПопробуйте позже или используйте Telegram Stars.",
            parse_mode="Markdown",
            reply_markup=payment_methods_keyboard()
...
async def process_pre_checkout(pre_checkout_query: PreCheckoutQuery, bot: Bot):
    ...
    try:
        ...
        logging.error(f"Ошибка в process_pre_checkout: {e}")

@router.message(F.successful_payment)
async def process_successful_payment(message: Message, bot: Bot):
    """Обработка успешного платежа Stars"""
    try:
        logging.info("=== НАЧАЛО ОБРАБОТКИ ПЛАТЕЖА ===")
        
        payment = message.successful_payment
        user_id = message.from_user.id
        amount = payment.total_amount
        
        logging.info(f"Платеж от user_id={user_id}, amount={amount}, currency={payment.currency}")
        ...
        logging.info("Платеж сохранен в БД")
        ...
        logging.info(f"Бонусные дни: {bonus_days}")
        ...
        if bonus_days > 0:
            ...
            end_date_str = new_end.strftime('%d.%m.%Y')
            logging.info(f"Подписка активирована до: {end_date_str}")
            
            await message.answer(
                f"✅ *Спасибо за поддержку!*\n\n"
                f"Сумма: {amount} Stars\n"
                f"Активная поддержка до: {end_date_str}\n\n"
        ...
        try:
            ...
            print(f"Donation: {first_name} (@{username}) - {amount} Stars")
        except:
            print(f"Donation: user {user_id} - {amount} Stars")
        ...
        # Отправляем пользователю простое сообщение об ошибке
        try:
            await message.answer(
                "✅ *Платеж получен!*\n\n"
